...
describe("popupContentReducer", () => {
  it("returns the default state when no state is provided", () => {
    expect(popupContentReducer(undefined, { type: "@@INIT" })).toEqual(
      defaultState,
    );
  });
  ...
  it("replaces state with the payload on SET_POPUP_CONTENT", () => {
    ...
    expect(
      popupContentReducer(defaultState, { type: "SET_POPUP_CONTENT", payload }),
    ).toBe(payload);
  });
  ...
  it("returns the current state unchanged for an unknown action", () => {
    ...
    expect(popupContentReducer(current, { type: "SOME_OTHER_ACTION" })).toBe(
      current,
    );
  });
});
